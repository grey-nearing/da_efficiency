...
Ndata = outdata.Ndata;
Isites = find(Ndata(:,1,1) >= Nmin);

% pull data from transport structure
Mopen = outdata.Mopen(1:Nmin,Isites,:,:);
Menkf = outdata.Menkf(1:Nmin,Isites,:,:);
Senkf = outdata.Senkf(1:Nmin,Isites,:,:);
Mback = outdata.Mback(1:Nmin,Isites,:,:);
Sback = outdata.Sback(1:Nmin,Isites,:,:);
val   = outdata.val(1:Nmin,Isites);
obs   = outdata.obs(1:Nmin,Isites);
cdf   = outdata.cdf(1:Nmin,Isites);

% dimensions
[Ntimes,Nsites,Nobs,Nmod] = size(Menkf);
...
assert(min(fil(:))>0); assert(max(fil(:))<=1);
...
Xa = reshape(Menkf,[Ntimes*Nsites,Nobs,Nmod]); % posterior mean
Qa = reshape(Senkf,[Ntimes*Nsites,Nobs,Nmod]); % posterior var
Xb = reshape(Mback,[Ntimes*Nsites,Nobs,Nmod]); % posterior mean
Qb = reshape(Sback,[Ntimes*Nsites,Nobs,Nmod]); % posterior var
...
%     Isites = randperm(N,Nn); N = Nn;
%     X = X(Isites); Y = Y(Isites); Z = Z(Isites); C = C(Isites);
%     Xa = Xa(Isites); Qa = Qa(Isites);
%     Xb = Xb(Isites); Qb = Qb(Isites);
%     
%     %% *************************************************************************************
...
%         % force 0*log(~)=0
%         Isites = find(pc>0);
%         pa = pa(Isites); pb = pb(Isites); pc = pc(Isites); pd = pd(Isites); pz = pz(Isites);
%         plssy = plssy(Isites); plpsy = plpsy(Isites); plspy = plspy(Isites); plppy = plppy(Isites);
%         % plssc = plssc(I); plpsc = plpsc(I); plspc = plspc(I); plppc = plppc(I);
%         pppp = pppp(Isites); ppss = ppss(Isites); ppsp = ppsp(Isites); ppps = ppps(Isites);
%         
...
%     
%     KLdata(Isites) = []; KLbaye(Isites) = [];
%     KLlyss(Isites) = []; KLlysp(Isites) = []; KLlyps(Isites) = []; KLlypp(Isites) = [];
%     %KLlcss(I) = []; KLlcsp(I) = []; KLlcps(I) = []; KLlcpp(I) = [];
%     KLpppp(Isites) = []; KLppss(Isites) = []; KLppsp(Isites) = []; KLppps(Isites) = [];
%     
