...
! SY: NOTE: Remember to change: [1] nsites value
!                               [2] name of sites list file (file 12)
...
! nldas grid  
 ...
 integer, parameter :: nsites = 129 !383 ! 129
...
! site info
 ...
 integer :: fid, gid
 character*1000 :: fname_site, gname_site
 ...
 real :: llat, llon
 real :: elev, tbot
 real, dimension(12) :: green

! param file info
 character*1000 :: fname
 integer :: ftn, ios, varid
 real, allocatable, dimension(:,:,:) :: var_green, landcover, soiltype
 real, allocatable, dimension(:,:) :: var_slp, var_tbot, var_elev, var_msk, var_lat, var_lon
 integer :: nlat, nlon

! read scan site lists
 in_domain = .false.
 open(12,name='../../scan/SCAN_Site_List.txt')
! open(12,name='../../../evaluation/sparse/List_Sites_SCAN_CRN_COSMOS.txt')
 do i = 1,nsites
   read(12,*) dummy, lat(i),lon(i)
...
! number of grid cells
 ...
 allocate(var_green(nlon,nlat,12))
...
print*, fname
 ...
 if (ios.ne.0) stop 113
 ios = nf90_get_var(ftn,varid,var_elev)
 if (ios.ne.0) stop 114
 write(6,*) 'Read Elevation'

 ! read lat
 ios = nf90_inq_varid(ftn,'lat',varid)
 if (ios.ne.0) stop 115
 ios = nf90_get_var(ftn,varid,var_lat)
 if (ios.ne.0) stop 116
 write(6,*) 'Read Latitude'

 ! read lat
 ios = nf90_inq_varid(ftn,'lon',varid)
 if (ios.ne.0) stop 117
 ios = nf90_get_var(ftn,varid,var_lon)
 if (ios.ne.0) stop 118
 write(6,*) 'Read Longitude'

 ! read greeness
 ios = nf90_inq_varid(ftn,'GREENNESS',varid)
 if (ios.ne.0) stop 119
 ios = nf90_get_var(ftn,varid,var_green)
 if (ios.ne.0) stop 120
 write(6,*) 'Read Monthly Greenness'
 ...
 do i = 1,nsites
   if (in_domain(i)) then
     ...
     if (i.lt.10) then
       ...
       gname_site = './site_data/shdfac_'//fn_site1
       gname_site = trim(gname_site)//'.txt'
     elseif (i.lt.100) then
       write(fn_site2,'(i2)') i
       fname_site = './site_data/parms_'//fn_site2
       fname_site = trim(fname_site)//'.txt'
       gname_site = './site_data/shdfac_'//fn_site2
       gname_site = trim(gname_site)//'.txt'
     else
       write(fn_site3,'(i3)') i
       fname_site = './site_data/parms_'//fn_site3
       fname_site = trim(fname_site)//'.txt'
       gname_site = './site_data/shdfac_'//fn_site3
       gname_site = trim(gname_site)//'.txt'
     endif
     fid = i + 12
     open(fid,name=fname_site,status='replace')
     open(gid,name=gname_site,status='replace')
     ...
     green = var_green(lon_id(i),lat_id(i),:)
     write(6,*) 'Read Greenness @ Site:',i,green
